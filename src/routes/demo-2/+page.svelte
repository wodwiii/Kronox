<script lang="ts">
    ...
    let products: string[] = [];
    let companyName: string = "TechMart Store";
    let customerServiceHours: string = "9:00 AM - 6:00 PM EST";
    let showSetupForm = true;
    let productInputs: {name: string, price: number, stock: number, description: string}[] = [
        {name: "Smart Watch Pro", price: 299.99, stock: 45, description: "Latest smartwatch featuring advanced health tracking, heart rate monitoring, sleep analysis, GPS tracking, water resistance up to 50m, and 5-day battery life"},
        {name: "Wireless Earbuds", price: 129.99, stock: 120, description: "Premium noise cancelling earbuds with 24-hour battery life, IPX5 water resistance, touch controls, and high-definition audio with deep bass response"},
        {name: "Laptop Elite", price: 999.99, stock: 30, description: "15-inch laptop with Intel i7 12th gen processor, 16GB RAM, 512GB SSD, NVIDIA RTX 3060 graphics, and stunning 4K display with 100% sRGB coverage"},
        {name: "Phone Case", price: 24.99, stock: 200, description: "Military-grade protective case for smartphones with shock absorption technology, raised bezels for screen protection, wireless charging compatible, and premium soft-touch finish"},
        {name: "Power Bank", price: 49.99, stock: 85, description: "20000mAh portable charger with fast charging support, dual USB ports, USB-C input/output, LED power indicator, and compact aluminum design"}
    ];
    ...
    const addProductInput = () => {
        productInputs = [...productInputs, {name: "", price: 0, stock: 0, description: ""}];
    };
    ...
    const startDemo = () => {
        if (!companyName || !customerServiceHours || 
            productInputs.some(p => !p.name || p.price <= 0 || p.stock < 0)) {
            alert("Please fill in all required fields");
            return;
        }
        
        products = productInputs.map(p => `${p.name} (Price: $${p.price}, Stock: ${p.stock}, Description: ${p.description})`);
        showSetupForm = false;
        initializeCanvas();
        initializeAudio();
    };
			...
			<div class="flex h-screen flex-col items-center justify-center p-6">
				...
				>
					<div class="flex flex-col items-center">
						...
						<p class="text-center text-white/70">Setting up your Customer Service AI Agent...</p>
						<p class="mt-4 text-center text-sm text-white/70">
							This is a demo of how the AI would handle customer service inquiries. Please act as a customer.
						</p>
					</div>
				</div>
			</div>
		{:else if showSetupForm}
			<div class="flex flex-col items-center justify-center p-6">
				<div class="w-full max-w-2xl rounded-2xl border border-white/10 bg-white/10 p-8 shadow-2xl backdrop-blur-xl">
					<h2 class="mb-6 text-2xl font-semibold text-white">Setup Customer Service Agent</h2>
					
					<div class="mb-6 rounded-lg bg-yellow-500/10 p-4 text-yellow-300">
						<p class="text-sm">Note: This manual input form is for demonstration purposes only. In a production environment, this data would be automatically pulled from your product management system.</p>
					</div>

					<div class="mb-4">
						<label class="mb-2 block text-sm font-medium text-white">Store Name</label>
						<input
							type="text"
                    ...
                    <div class="mb-4">
                        <label class="mb-2 block text-sm font-medium text-white">Customer Service Hours</label>
                        <input
                            type="text"
                            bind:value={customerServiceHours}
                            class="w-full rounded-lg border border-white/10 bg-white/5 p-2.5 text-white"
					...
					<div class="mb-4">
						<label class="mb-2 block text-sm font-medium text-white">Products</label>
					...
					<div class="mx-auto flex max-w-4xl items-center justify-between">
						<div class="flex items-center space-x-4">
							...
							<div class="flex items-center space-x-3">
								...
								<h1 class="text-xl font-semibold text-white">Customer Service Agent</h1>
							</div>
						</div>
						<div
