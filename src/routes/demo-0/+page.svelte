<script lang="ts">
    ...
    let additionalInfo = "Handle with care - Contains sensitive electronic components";
    let productInputs: {name: string, quantity: number}[] = [
        {name: "4K LCD Display Panels", quantity: 50},
        {name: "Wireless Router Modules", quantity: 200}, 
        {name: "SSD Storage Drives 1TB", quantity: 150},
        {name: "RGB Mechanical Keyboards", quantity: 100},
        {name: "Gaming Graphics Cards", quantity: 75}
    ];
    ...
    const addProductInput = () => {
        productInputs = [...productInputs, {name: "", quantity: 0}];
    };
    ...
    const startDemo = () => {
        if (!companyName || productInputs.some(p => !p.name || p.quantity <= 0)) {
            alert("Please fill in all required fields");
            return;
        }
        
        products = productInputs.map(p => `${p.name} (Quantity: ${p.quantity})`);
        showSetupForm = false;
        initializeCanvas();
        initializeAudio();
    };
				...
				>
					<div class="flex flex-col items-center">
						...
						<p class="mt-4 text-center text-sm text-white/70">
							This is a demo of how the AI would handle product restocking. Please act as the supplier.
						</p>
					</div>
				</div>
			</div>
		{:else if showSetupForm}
			<div class="flex flex-col items-center justify-center p-6">
				<div class="w-full max-w-2xl rounded-2xl border border-white/10 bg-white/10 p-8 shadow-2xl backdrop-blur-xl">
					<h2 class="mb-6 text-2xl font-semibold text-white">Setup Demo Call</h2>
					
					<div class="mb-6 rounded-lg bg-yellow-500/10 p-4 text-yellow-300">
						<p class="text-sm">Note: This manual input form is for demonstration purposes only. In a production environment, this data would be automatically pulled from your inventory management system.</p>
					...
					<div class="mb-6">
						<label class="mb-2 block text-sm font-medium text-white">Additional Information</label>
						<textarea
					...
					<div class="mx-auto flex max-w-4xl items-center justify-between">
						<div class="flex items-center space-x-4">
							...
							<div class="flex items-center space-x-3">
								...
								<h1 class="text-xl font-semibold text-white">Product Restocking Agent</h1>
							</div>
						</div>
						<div
